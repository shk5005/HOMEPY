...
- **Capex → Revenue → FCF 전환율**: 인프라 국면에서는 "1달러의 AI 인프라가 미래 몇 달러의 FCF를
...
사용자가 "대시보드", "인터랙티브", "시각화", "한눈에", "정리해줘(시각적)"를 요청하면 위 텍스트
진단을 **단일 HTML 아티팩트**로 렌더한다. 목적은 "S-curve상 어디인가"를 즉시 읽히게 하는 것이므로
**S-curve 위치 그래프를 히어로로** 둔다.

렌더 전 `artifact-design`(디자인 투자 수준 보정)과, 차트가 있으므로 `dataviz`(팔레트·마크 규칙)를
먼저 로드한다. 그다음 아래 순서로 구성한다:

1. **히어로 — AI 채택 S-Curve 위치도**: 7단계(발명→…→포화) 곡선 위에 대상의 사업부/기술을
   좌표로 표시(마커 hover = 상세). 변곡 구간(⑤→⑥)을 하이라이트하고 "현재 핵심 위치"를 콜아웃.
   세로축이 **채택량**임을 명시하고, **가격/수익 사이클은 별개 축**이면 별도 경고 마커(▼)로 분리.
2. **곱셈 게이트** S×M×E 타일(약한 항을 시각적으로 강조) → Alpha 결과.
3. **6-레이어 점수 바**(hover=근거, [라벨] 병기).
4. **Δ 오버레이** 시그널 카드(가속▲=good / 감속▼=critical, 상태색은 액센트와 분리).
5. **Red Team 5** 칩(해당/비해당/확인불가 색상).
6. **종합 판정 + 매수/반대 논거 + 트리거**.
7. **데이터 무결성 푸터**: 4라벨 고지 + 실현 불가·이상치는 "인용 배제" 경고 박스로 명시.

제약: 자기완결형(외부 CDN·폰트 금지, 인라인), 3-state 테마 토큰(시스템/라이트/다크), 한글 폴백
폰트 스택, 데이터 컬럼 `tabular-nums`. **대시보드도 텍스트 진단과 동일하게 아부 0%·4라벨을
지키며, 시각화가 미확인 수치를 확정처럼 보이게 만들지 않도록** [미검증]/[확인 불가]를 화면에 노출한다.
